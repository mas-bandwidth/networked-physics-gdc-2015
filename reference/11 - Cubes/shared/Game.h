...
//			printf( "initializing game world\n" );
		...
		{
			NET_ASSERT( id > 0 );
			NET_ASSERT( id < (ObjectId) config.maxObjects );
		...
		{
			NET_ASSERT( activeIndex >= 0 );
			NET_ASSERT( activeIndex < activeObjects.GetCount() );
	...
	protected:
		...
		void ProcessPlayerInput( int playerId, float deltaTime )
		{
			...
			{			
				...
				math::Vector pull_origin = push_origin;
				push_origin.z = -0.1f;
