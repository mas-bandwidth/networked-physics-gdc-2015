...
File.readlines( 'output/position_values.txt' ).each do |line|
...
class PositionBandwidthEstimate
  ...
  def evaluate position_values, position_base_values
    ...
    large_threshold_xy = ( 1 << ( @large_bits_xy - 1 ) ) - 1;
    small_threshold_z = ( 1 << ( @small_bits_z - 1 ) ) - 1;
    large_threshold_z = ( 1 << ( @large_bits_z - 1 ) ) - 1;
  ...
  for large_bits_xy in 8..10
    for small_bits_z in 2..6
      for large_bits_z in 8..10
        puts "#{small_bits_xy}-#{large_bits_xy}-#{small_bits_z}-#{large_bits_z}"
...
for i in 0..9
  ...
  puts "compression #{bandwidth_estimate.small_bits_xy}-#{bandwidth_estimate.large_bits_xy}-#{bandwidth_estimate.small_bits_z}-#{bandwidth_estimate.large_bits_z}: #{bandwidth_estimate.total_bits}"
end
