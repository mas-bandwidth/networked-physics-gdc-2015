...
void test_message_factory()
{
    ...
    core::memory::initialize();
    {
        ...
        TestMessage * testMessage = (TestMessage*) messageFactory.Create( MESSAGE_TEST );
        CORE_CHECK( testMessage );
        CORE_CHECK( testMessage->GetRefCount() == 1 );
        CORE_CHECK( testMessage->GetType() == MESSAGE_TEST );

        protocol::BlockMessage * blockMessage = (protocol::BlockMessage*) messageFactory.Create( MESSAGE_BLOCK );
